...
def receiveOnePing(icmpSocket, destinationAddress, ID, timeout):
    ...
    try:
        ...
        # Extract TTL from the IP header (the 9th byte)
        ttl = struct.unpack('B', data[8:9])[0]
        return data, time_received, ttl
...
def doOnePing(destinationAddress, timeout):
    try:
        ...
        response, time_received, ttl = receiveOnePing(icmpSocket, destinationAddress, ID, timeout)
...
def ping(host, timeout=1, count=4):
    try:
        ...
        for _ in range(count):
            ...
            network_delay, ttl = doOnePing(destinationAddress, timeout)
            if network_delay is not None:
                packets_sent += 1
                packets_received += 1
                total_delay += network_delay
                print(f"Reply from {destinationAddress}: bytes=32 time={int(network_delay)}ms TTL={ttl}")
